...
function statusReadyText() {
  return `IndexedDB ready | ${state.products.length} products loaded`;
}
...
async function startCamera() {
  ...
  const config = {
    fps: 10,
    qrbox: (vw, vh) => {
      const size = Math.max(160, Math.min(320, Math.floor(Math.min(vw, vh) * 0.72)));
      return { width: size, height: size };
    }
  };

  try {
    await state.scanner.start({ facingMode: { ideal: 'environment' } }, config, onDecode);
    setStatus('Camera running | point at barcode');
  } catch (primaryErr) {
    try {
      const cameras = await Html5Qrcode.getCameras();
      if (!cameras.length) throw primaryErr;
      await state.scanner.start(cameras[0].id, config, onDecode);
      setStatus('Camera running | point at barcode');
    } catch (err) {
      ...
      state.scanner = null;
      $('#reader').innerHTML = '';
      setStatus('Camera unavailable. Use Scan Image or manual input.');
    }
  }
}
...
async function scanImage(file) {
  ...
  if (state.scanner) await stopCamera();
  ...
  try {
    ...
    setStatus('Image scan failed. Try a clearer photo or manual input.');
  } finally {
